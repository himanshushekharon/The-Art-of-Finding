...
//   map = new google.maps.Map(document.getElementById('map'), {
//     zoom: 13,
//     // Updated coordinates for Delhi, India
//     center: { lat: 28.6139, lng: 77.2090 }
//   });

//   // Updated marker for Delhi
//   addMarker(28.6139, 77.2090, 'Delhi');
//   $('#galleryName').text('');
//   $('#galleryAddr').text('');
// }
...
$(document).ready(function () {
  ...
  function initializeQuery() {
    $.ajax({
      ...
      console.error('Failed to fetch Xapp-Token. Please check your client ID and secret.');
      alert('Error: Unable to fetch API token. Please try again later.');
    });
  }
  ...
  function getArtistId() {
    ...
    $.ajax({
      ...
      var artistResults = data._embedded.results.filter(function (item) {
      ...
      var artistId = artistResults[0]._links.self.href.split('/').pop();

      artistIdQueryFunc(artistId);

      artistName = artistResults[0].title;
    }).fail(function () {
      console.error('Failed to fetch artist data. Please check your API token or network connection.');
      alert('Error: Unable to fetch artist data. Please try again later.');
    });
